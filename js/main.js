...
getData('https://api.nytimes.com/svc/movies/v2/reviews/search.json?query=godfather&api-key=gNmZAMcqra1afb8nJTABRROcDD1tueje').then(function(result){
  if(result){
    ...
    if(result.results.length>8){
      ...
      let list= ``;
      for (let news of result.results){
        let date = new Date(news.date_updated);
        ...
        list += `   <div class="news-grid__item-img" style="background-image:url(${news.multimedia})">`;
        list += `   <div class="news-grid__item-data">${newDate } <br>${month}</div></div>`;
        list += `<a class="news-grid__item-title">${news.display_title}</a>`;
        if(news.byline==null){
          news.byline='Admin';
        }
        list += `<div class="news-grid__item-author">${news.byline}`;
